...
Convention-based: each folder in `apps/` is an app. `app.json` provides metadata (icon, variant, visibility, permissions) and, via its `description`, what the AI knows about the app by default — an `agent/prompt.md` overrides that for apps whose agent needs more.
