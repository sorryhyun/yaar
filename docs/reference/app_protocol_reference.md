...
| `list('yaar://windows/{windowId}/history')` | Every `app_command` sent to this window so far (successful or failed, with the sending agent), plus the server's own `replayed` / `restored` events — the replay log the coordinator has always kept, opened to agents. `read` the same URI for JSON, `read …/history/{seq}` for one entry's full params, and `invoke(…/history, { action: 'restore', upTo })` to forget everything after `upTo` and remount so the kept commands replay. It holds what agents did; user-produced state inside the window is not in it. |
...
#### The storage door is built in, up to the commons

Every app agent holds `storage:write` / `storage:delete` / `storage:list` and the relative `storage/{path}` spelling on `query`, with no declaration required. Two trees are behind that and both are already the app's: its own (`yaar://apps/{id}/storage/`, which no permission governs) and the commons (`yaar://storage/shared/`, granted to every app for being an app). The system prompt documents both for every app — the static app-scoped section plus a shared section rendered from that app's own declared entries — so what an agent is told and what the door admits agree by construction. The `query`/`command` tool *descriptions* mention storage nowhere; the prompt is the single source.

This was briefly gated on the manifest declaring an entry under `yaar://storage/`, own tree included. The gate was removed: no manifest ever declares the app's own tree (there is nothing to declare), so it disarmed exactly the apps that had done nothing unusual, and it split one app in half — the same app's **iframe** wrote that tree freely through `@bundled/yaar` the whole time.

**An app may override the built-ins.** A protocol command named `storage:read` / `storage:write` / `storage:delete` / `storage:list`, or any command carrying that name in `aliases`, is what the spelling runs for a **relative** path (`yaar://apps/self/…` included, since it normalizes to relative): `query({ stateKey: 'storage/x' })` and `command('storage:read', { path: 'x' })` both reach the app's handler with `{ path, ...params }`, and the answer is the app's, opened with a note naming the override. This is how an app whose files are renderings (a document saved as `.docx` / `.md` / `.json`) makes the generic verb do the right thing instead of shipping a second write command beside it. The commons overrides too — `yaar://storage/shared/…`, or its relative shortcut `shared/{path}`, costs no permission, so there is no gate for app code to stand in front of; the handler receives the commons URI as `path`. Any other `yaar://storage/…` path never overrides — the shared tree's `app.json` gate must stay between the agent and the bytes. `app/{path}` is the own tree spelled explicitly, and it is the spelling the door *emits* — every app-scoped listing entry and every write/delete receipt carries it (`appScopedRef`), so a relative path names its tree instead of reading like a bare storage-root one; a bare relative argument still resolves to the own tree, so nothing already spelled stops working (`mcp/app-agent/shared-storage.ts`, `expandStorageShortcut`, `namesCommons`). The built-in `storage:write` / `storage:delete` answer in kind for both trees — `{ uri, path, written, bytes }` / `{ uri, path, deleted }` — so a caller reads a structured reply whichever door it went through and never branches on path spelling to parse it. Resolver: `mcp/app-agent/storage-override.ts`; the declaration is the protocol itself, so there is no way to declare an override without implementing it.
...
When an iframe app reloads (e.g., due to HMR or navigation), the server detects a new `APP_PROTOCOL_READY` for a window that was already marked as ready, and replays its recorded `appCommands` so the app returns to its previous state (`AppWindowCoordinator.replayCommands()`). Four refinements on top of that:

- **Replayed commands run one at a time, in the order they were recorded.** The server sends the whole log in one go; the iframe SDK queues the frames stamped `replayed` and starts each handler only once the previous one has answered. Dispatched together they would interleave at the first `await` and the app would settle into whatever state the last write happened to win — devtools came back from a remount showing one project's file tree over another project's editor. A fresh command from an agent does not queue behind a replay: it is not part of that history. One replayed command that never answers stops holding the rest up after 30s.
- A command whose *currently running* registration declared `replay: 'never'` for it — on the command, or inherited from `defineApp({ replay: 'never' })`, which is how an app that restores itself from its own storage opts out wholesale — is skipped rather than resent — those are commands that append, notify, or otherwise have a one-shot effect, so resending would duplicate it rather than restore state. The replay policy is read from the registration that just came back up, not the one that was there before the reload.
