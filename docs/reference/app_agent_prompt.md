...
5. **App-authoring contract** — only for apps whose `app.json` bundles `yaar-dev` (today:
...
6. **Controllable apps** — rendered from `app.json`'s `controls`: the app ids this agent may
...
> state key and every command's call signature with its opening sentence. Pass `command`
> to get one command in full instead, with its complete parameter schema — that is the
> door to use when a signature leaves you unsure, not a second full describe. Omit appId
> to describe your own app; pass appId to inspect another app you are permitted to control.
