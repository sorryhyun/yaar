...
사용자는 코드를 직접 작성할 필요가 없습니다. AI가 샌드박스에서 TypeScript를 작성하고, esbuild로 컴파일하고, 프리뷰한 뒤 앱으로 배포합니다. 빌드된 앱은 모든 라이브러리와 코드가 하나의 HTML 파일로 번들링되므로, 별도의 설치 없이 어떤 브라우저에서든 독립적으로 실행할 수 있습니다.
...
| 도구 | 설명 |
|------|------|
| `write` | 샌드박스에 TypeScript 파일 작성 (`sandbox://` URI 사용) |
| `read` | 샌드박스 파일 읽기 (`sandbox://` URI 사용) |
| `list` | 샌드박스 파일 목록 조회 (`sandbox://` URI 사용) |
| `edit` | 샌드박스 파일 검색-치환 편집 (`sandbox://` URI 사용) |
...
| `set_config` | 앱 설정 저장 (`section: "app"`, `content: { appId, config }`) |
| `get_config` | 앱 설정 읽기 (`section: "app"`, `id?: appId`) |
| `remove_config` | 앱 설정 삭제 (`section: "app"`, `id: appId`) |
...
```
write(uri: "sandbox:///src/main.ts", content: "...")      // 새 샌드박스 자동 생성
write(uri: "sandbox://1739xxx/src/main.ts", content: "...") // 기존 샌드박스에 추가
```
...
edit(uri: "sandbox://{sandboxId}/path", old_string, new_string)  // 또는 write로 전체 교체
...
- **OAuth 클라이언트를 컴파일된 앱으로 만들지 마세요** — OAuth에는 서버 측 `client_secret` 토큰 교환이 필요합니다. 대신, 사용자가 개인 액세스 토큰(PAT)을 제공하고 `set_config(section: "app")`로 저장하는 API 기반 앱(SKILL.md만)을 만드세요.
...
옵션 A: API 기반 앱 (API 래퍼에 적합)
  ...
  사용자가 PAT 제공 → set_config(section: "app")로 저장
...
- `set_config(section: "app", content: { appId: "moltbook", config: { api_key: "..." } })` — 저장
- `get_config(section: "app", id: "moltbook")` — 읽기
- `remove_config(section: "app", id: "moltbook", key?: "api_key")` — 삭제
...
Users don't need to write code. The AI writes TypeScript in a sandbox, compiles with esbuild, previews the result, and deploys it as an app. Built apps are bundled into a single self-contained HTML file — all libraries, CSS, and code are inlined, so they can run independently in any browser with zero dependencies.
...
| Tool | Description |
|------|-------------|
| `write` | Write files to sandbox (`sandbox://` URI) |
| `read` | Read sandbox files (`sandbox://` URI) |
| `list` | List sandbox files (`sandbox://` URI) |
| `edit` | Apply search-and-replace edits to sandbox files (`sandbox://` URI) |
...
| `set_config` | Save app config (`section: "app"`, `content: { appId, config }`) |
| `get_config` | Read app config (`section: "app"`, `id?: appId`) |
| `remove_config` | Remove app config (`section: "app"`, `id: appId`) |
...
```
write(uri: "sandbox:///src/main.ts", content: "...")        // new sandbox auto-created
write(uri: "sandbox://1739xxx/src/main.ts", content: "...") // write to existing sandbox
```

- Creates files in an isolated sandbox directory
- `sandbox:///path` (triple slash) auto-generates a new sandbox ID
...
edit(uri: "sandbox://{sandboxId}/path", old_string, new_string)  // or write for full replacement
compile(sandbox: sandboxId)
...
- `set_config(section: "app", content: { appId: "moltbook", config: { api_key: "..." } })` — save
- `get_config(section: "app", id: "moltbook")` — read
- `remove_config(section: "app", id: "moltbook", key?: "api_key")` — remove
