...
| Three.js WebGPU | `@bundled/three/webgpu`, `@bundled/three/tsl` | `WebGPURenderer` (falls back to WebGL2 by itself), node materials, TSL shaders. Needs `"three": "webgpu"` in `app.json`, and then `@bundled/three` itself resolves to the WebGPU build — keep importing `* as THREE from '@bundled/three'`. An app runs one three.js: `WebGLRenderer`, GLSL `ShaderMaterial` and `onBeforeCompile` are gone in a WebGPU app |
...
|-----|------------|---------|------------------------|
| Dev Tools | `@bundled/yaar-dev` | `compile()`, `typecheck()`, `findReferences()` (symbol references and callers via the TypeScript language service), `deploy()`, `bundledLibraries()`, and per-app version history: `gitHistory()`, `gitDiff()`, `gitRestore()`, `gitCheckpoint()` | `"yaar-dev"` |
...
| Media | `@bundled/yaar-media` | `mediaUrl(url, { referer? })` — a same-origin URL for `<video src>`/`<audio src>`/`fetch()` that streams through `/api/media-proxy` with Range passthrough (no 10MB base64 envelope). YouTube audio download via the server's optional yt-dlp binary: `ytdlpStatus()`, `resolveMedia()`, `downloadAudio()`, `cancelDownload()`. The bundle alone grants the capability — no `permissions` entry, app manifests never hold `yaar://system/*` URIs. Files land in `shared/media/` | `"yaar-media"` |
...
|---|---|
| `createCollapsiblePanel({ pinKey, closeDelayMs })` | Hover-expand + pin sidebar. `expanded()`, `pinned()`, `open()`, `scheduleClose()`, `close()`, `cancelClose()`, `togglePin()`, `setPin(v)`, `setResizing(active)`. Pin state persists when `pinKey` is given. Two predicates cover reasons the panel doesn't own: `canOpen` is consulted by `open()` (return `false` while a drag from elsewhere sweeps the rail), `holdOpen` when the fold *fires* (return `true` while a field inside has focus, then `scheduleClose()` on blur). `drawer: isNarrow` makes it a modal drawer on a narrow window — `expanded()` ignores the pin and the hover fold is inert, so only `open()`/`close()` move it; `drawer()` reports the mode |
| `isNarrow()` / `isTouch()` / `createMediaQuery(q)` | Reactive media-query signals. `isNarrow` (window ≤768px: one column, drawer instead of side panel) and `isTouch` (coarse pointer: no hover, 44px targets) read the same queries as the injected stylesheet (`NARROW_QUERY` / `TOUCH_QUERY`), so JS and CSS agree on the layout |
...
| `controls` | `(string \| { appId, commands?, minimized? })[]` | Other apps this app may drive. A target with no window on the caller's monitor gets one opened; `minimized: true` opens it minimized. **Bundled apps only** |
