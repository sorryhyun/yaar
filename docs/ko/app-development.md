...
"테트리스 만들어줘"
    ...
    ↓  AI가 코드 작성    invoke('yaar://sandbox/new/src/main.ts', { action: "write", content: "..." })
    ↓  컴파일             invoke('yaar://sandbox/{id}', { action: "compile" })
    ↓  iframe 윈도우로 프리뷰
    ↓  바탕화면에 배포    invoke('yaar://sandbox/{id}', { action: "deploy", appId: "tetris", ... })
...
| 동사 | URI | 설명 |
|------|-----|------|
| `invoke` | `yaar://sandbox/new/src/main.ts`, `{ action: "write", content }` | 새 샌드박스에 파일 작성 (ID 자동 생성) |
| `invoke` | `yaar://sandbox/{id}/src/main.ts`, `{ action: "write", content }` | 기존 샌드박스에 파일 작성 |
| `invoke` | `yaar://sandbox/{id}/src/main.ts`, `{ action: "edit", old_string, new_string }` | 검색-치환 편집 |
| `read` | `yaar://sandbox/{id}/src/main.ts` | 샌드박스 파일 읽기 |
| `list` | `yaar://sandbox/{id}/` | 샌드박스 파일 목록 조회 |
| `invoke` | `yaar://sandbox/{id}`, `{ action: "compile" }` | `src/main.ts` → 단일 HTML로 번들 (Bun) |
| `invoke` | `yaar://sandbox/{id}`, `{ action: "typecheck" }` | TypeScript 타입 검사 |
| `invoke` | `yaar://sandbox/{id}`, `{ action: "deploy", appId, ... }` | 컴파일된 앱을 바탕화면에 배포 |
| `invoke` | `yaar://sandbox/new`, `{ action: "clone", uri }` | 배포된 앱의 소스를 새 샌드박스로 복제 |
| `invoke` | `yaar://sandbox/eval`, `{ code }` | JavaScript를 샌드박스 VM에서 실행 |
...
| `read` | `yaar://skills/{topic}` | 참조 문서 로드 (`app_dev`, `sandbox`, `components`, `host_api`, `app_protocol`) |
...
```
invoke('yaar://sandbox/new/src/main.ts', { action: "write", content: "..." })        // 새 샌드박스 자동 생성
invoke('yaar://sandbox/1739xxx/src/main.ts', { action: "write", content: "..." })     // 기존 샌드박스에 추가
```

- 격리된 샌드박스 디렉토리에 파일 생성
- `yaar://sandbox/new/{path}` 사용 시 ID 자동 생성
...
```
invoke('yaar://sandbox/new', { action: "clone", uri: "yaar://apps/my-app" })  → sandboxId 반환
invoke('yaar://sandbox/{sandboxId}/src/main.ts', { action: "edit", old_string: "...", new_string: "..." })
invoke('yaar://sandbox/{sandboxId}', { action: "compile" })
invoke('yaar://sandbox/{sandboxId}', { action: "deploy", appId: "my-app" })  // 동일 appId로 덮어쓰기
```
...
`invoke('yaar://sandbox/eval', { code })`는 격리된 VM에서 코드를 실행합니다.
