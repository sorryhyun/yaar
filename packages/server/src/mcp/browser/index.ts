/**
 * MCP browser tools — visible browser automation via Playwright.
 *
...
async function refreshWindow(session: BrowserSession): Promise<void> {
  ...
  await actionEmitter
    .emitAppProtocolRequest(
      session.windowId,
      {
        kind: 'command',
        command: 'refresh',
        params: { url: session.currentUrl, title: session.currentTitle },
      },
      5000,
    )
    .catch(() => {
  ...
  server.registerTool(
    ...
    async (args) => {
      ...
      if (!session) {
        ...
        // Wait for app to register, then send initial state
