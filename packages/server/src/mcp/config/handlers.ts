...
function assertConfig(resolved: ResolvedUri): asserts resolved is Extract<ResolvedUri, { kind: 'config' }> {
...
export function registerConfigHandlers(registry: ResourceRegistry): void {
  ...
  registry.register('yaar://config/settings', {
    ...
    async invoke(_resolved: ResolvedUri, payload?: Record<string, unknown>): Promise<VerbResult> {
      if (!payload) return { content: [{ type: 'text', text: 'Payload required for settings update.' }], isError: true };
  ...
  registry.register('yaar://config/hooks', {
    ...
    async invoke(_resolved: ResolvedUri, payload?: Record<string, unknown>): Promise<VerbResult> {
      if (!payload) return { content: [{ type: 'text', text: 'Payload required to create a hook.' }], isError: true };
  ...
  registry.register('yaar://config/hooks/*', {
    ...
    async read(resolved: ResolvedUri): Promise<VerbResult> {
      ...
      if (!hook) return { content: [{ type: 'text', text: `Hook "${hookId}" not found.` }], isError: true };
      return ok(JSON.stringify(hook, null, 2));
    },

    async delete(resolved: ResolvedUri): Promise<VerbResult> {
      assertConfig(resolved);
      if (!resolved.id) return { content: [{ type: 'text', text: 'Hook ID required.' }], isError: true };
  ...
  registry.register('yaar://config/shortcuts', {
    ...
    async invoke(_resolved: ResolvedUri, payload?: Record<string, unknown>): Promise<VerbResult> {
      if (!payload) return { content: [{ type: 'text', text: 'Payload required.' }], isError: true };
  ...
  registry.register('yaar://config/shortcuts/*', {
    ...
    async delete(resolved: ResolvedUri): Promise<VerbResult> {
      assertConfig(resolved);
      if (!resolved.id) return { content: [{ type: 'text', text: 'Shortcut ID required.' }], isError: true };
  ...
  registry.register('yaar://config/mounts/*', {
    ...
    async delete(resolved: ResolvedUri): Promise<VerbResult> {
      assertConfig(resolved);
      if (!resolved.id) return { content: [{ type: 'text', text: 'Mount alias required.' }], isError: true };
  ...
  registry.register('yaar://config/app/*', {
    ...
    async read(resolved: ResolvedUri): Promise<VerbResult> {
      assertConfig(resolved);
      if (!resolved.id) return { content: [{ type: 'text', text: 'App ID required.' }], isError: true };
