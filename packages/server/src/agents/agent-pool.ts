 ...
 * idleness (`AppAgentRegistry.reapIdle`), `fresh:true`, monitor removal, explicit delete,
 ...
 * - App agents: persistent per (monitor, app), handle app protocol communication — the
 *   whole tier lives in {@link AppAgentRegistry}, reached through the {@link appAgents} door
...
export class AgentPool {
  ...
  /**
   * The app-agent tier — one persistent agent per (monitor, app), with its idle reaper
   * and spawn reservations. Public because it is the door, not a pass-through:
   * everything about that tier (the reuse, the reaper, the settle-before-sweep) is the
  ...
  // ── App agents ───────────────────────────────────────────────────
  //
  // The tier itself — creation, reuse, idle reaping, per-app disposal — lives in
  // {@link AppAgentRegistry}, reached through the {@link appAgents} door. What stays
  // here is the one operation that spans two tiers:

  /**
   * Dispose every app agent *and every sub-agent* owned by a monitor. Both belong to
   * the monitor whose windows they drive, so tearing the monitor down reclaims them —
   * nothing else would, and leaking them would hold limiter slots for a monitor
   * that's gone.
   *
   * The two sweeps are deliberately not folded into one walk of the app tier: a
   * persona is spawned by an app's *iframe*, which needs no app agent to exist. An
   * app that only ever talks to its personas has entries in the sub-agent registry
   * and none in the app registry, so walking app agents would reclaim nothing and
   * leak every slot the monitor's personas hold.
   */
  async disposeAppAgentsForMonitor(monitorId: string): Promise<void> {
    await this.appAgents.disposeForMonitor(monitorId);
  ...
  async cleanup(): Promise<void> {
    // Before the settle/snapshot below, so a sweep cannot fire concurrently with the
    // teardown walking the same agents.
    this.appAgents.stopSweep();
    ...
    await this.appAgents.settleSpawns(() => true);
