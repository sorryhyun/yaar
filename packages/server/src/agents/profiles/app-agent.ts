 ...
 * protocol manifest, with access only to app_query, app_command, and relay tools.
 */
...
- **app_query** — read the app's state (pass a stateKey, or omit for the manifest)
- **app_command** — execute an action in the app (pass command name and params)
- **relay** — hand off a message to the main agent when the request is outside your domain
`;
...
- Use app_query to read state before making changes
- Use app_command to execute actions
- If the user's request is outside your app's domain, use relay to hand off to the main agent
