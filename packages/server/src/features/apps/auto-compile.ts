...
import { listAppDirs } from './roots.js';
...
export async function autoCompileApps(
  ...
  // Apps with src/main.ts, resolved the way every other lookup resolves them — a
  // user-installed copy shadowed by a bundled one is not the app, so it is not built.
  const appDirs: { appId: string; appPath: string }[] = [];
  for (const { appId, dir } of await listAppDirs()) {
    if (await Bun.file(join(dir, 'src', 'main.ts')).exists()) appDirs.push({ appId, appPath: dir });
  }
  ...
  const tasks = selected.map(({ appId, appPath }) => async () => {
    try {
      ...
      // Title and bundles default from the app's own app.json.
      const compileResult = await compileTypeScript(appPath);
