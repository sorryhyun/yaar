...
import { listAppDirs, resolveApp, resolveAppDir, type AppSource } from './roots.js';
import {
  readManifest,
  type AppKind,
  type ControlEntry,
...
export async function subAgentDenialReason(
  ...
  const manifest = appDir ? await readManifest(appDir) : null;
  if (!manifest) return 'not-declared';
  if (manifest.usesRetiredPersonasKey) return 'retired-key';
  return manifest.subagents ? 'not-approved' : 'not-declared';
}
...
/** Build an AppInfo for a single app directory. */
async function readAppInfo(appPath: string, appId: string, source: AppSource): Promise<AppInfo> {
  ...
  // A missing or malformed app.json leaves every field at its default.
  const meta = await readManifest(appPath);

  let icon = meta?.icon;
  let iconType: 'emoji' | 'image' | undefined = icon ? 'emoji' : undefined;
  ...
  const kind: AppKind = meta?.kind === 'system' && source === 'bundled' ? 'system' : 'app';
  ...
  const controls = source === 'bundled' ? meta?.controls : undefined;

  const permissions = meta?.permissions
    ? narrowForeignStorage(meta.permissions, appId, source)
    : undefined;
  ...
  if (meta?.run) {
    ...
    resolvedRun = meta.run.startsWith('/')
      ? meta.run
      : buildYaarUri('apps', `${appId}/${meta.run}`);
  } else if (isCompiled) {
  ...
  return {
    ...
    ...(meta?.createShortcut === false && { createShortcut: false }),
    ...(resolvedRun && { run: resolvedRun }),
    isCompiled,
    ...(protocol && { protocol }),
    ...(meta?.variant && { variant: meta.variant }),
    ...(meta?.dockEdge && { dockEdge: meta.dockEdge }),
    ...(meta?.frameless && { frameless: true }),
    ...(meta?.windowStyle && { windowStyle: meta.windowStyle }),
    ...(meta?.defaultWidth && { defaultWidth: meta.defaultWidth }),
    ...(meta?.defaultHeight && { defaultHeight: meta.defaultHeight }),
    ...(permissions && { permissions }),
    ...(meta?.agentType && { agentType: meta.agentType }),
    ...(controls && controls.length > 0 && { controls }),
    ...(meta?.bundles && meta.bundles.length > 0 && { bundles: meta.bundles }),
  };
}
...
// pass. Mutations (install/uninstall/deploy/restore) go through notifyAppChanged
// (`changed.ts`), which drops this so their changes show up immediately; the TTL is
// a backstop for anything not explicitly invalidated.
...
export async function listApps(): Promise<AppInfo[]> {
  ...
  const apps: AppInfo[] = [];
  for (const { appId, dir, source } of await listAppDirs()) {
    apps.push(await readAppInfo(dir, appId, source));
  }
...
/**
 * What an app window and the doors it calls need to know about an app: its manifest,
 * narrowed to what the app's source and the user's install-time grant entitle it to.
 */
...
/**
 * The effective manifest of an installed app, or null when the app is absent, has no
 * readable app.json, or declares nothing this reports.
 *
...
export async function getAppMeta(appId: string): Promise<AppMeta | null> {
  ...
  if (!app) return null;
  const { dir: appDir, source } = app;
  const meta = await readManifest(appDir);
  if (!meta) return null;

  const result: AppMeta = {};
  if (meta.messaging) result.messaging = meta.messaging;
  // Only bundled apps may be `system` (see readAppInfo) — an installed app
  // can't claim it by shipping kind:"system" in its manifest.
  if (meta.kind === 'system' && source === 'bundled') result.systemApp = true;
  // Only bundled apps may declare authority to drive other apps (see readAppInfo).
  if (meta.controls && meta.controls.length > 0 && source === 'bundled') {
    result.controls = meta.controls;
  }
  if (meta.variant) result.variant = meta.variant;
  if (meta.dockEdge) result.dockEdge = meta.dockEdge;
  if (meta.frameless) result.frameless = true;
  if (meta.windowStyle) result.windowStyle = meta.windowStyle;
  if (meta.defaultWidth !== undefined) result.defaultWidth = meta.defaultWidth;
  if (meta.defaultHeight !== undefined) result.defaultHeight = meta.defaultHeight;
  ...
  if (meta.bundles && meta.bundles.length > 0) result.bundles = meta.bundles;
  ...
  //
  // Say so here, where every consumer of the effective manifest passes, rather than
  // leaving the author to discover it when a spawn fails much later.
  if (meta.usesRetiredPersonasKey) {
    log.warn(
  ...
  const granted = applyGrant(source, source === 'bundled' ? null : await readAppGrant(appId), {
    ...
    ...(meta.streams && meta.streams.length > 0 ? { streams: meta.streams } : {}),
  });
  if (granted.streams) result.streams = granted.streams;
  if (granted.subagents) result.subagents = granted.subagents;
  if (await Bun.file(join(appDir, 'dist', 'protocol.json')).exists()) result.hasProtocol = true;
...
export async function agentDocPathsFor(appDir: string): Promise<string[]> {
  ...
  const paths = agentDocPaths((await readManifest(appDir))?.raw ?? null);
...
async function resolveAgentDocPath(appId: string, kind: AgentDocKind): Promise<string> {
  const appDir = resolveAppDir(appId);
  const manifest = appDir ? await readManifest(appDir) : null;
  return agentDocPaths(manifest?.raw ?? null)[kind];
}
...
export async function loadAllAppHints(): Promise<{ appId: string; hint: string }[]> {
  const hints = await Promise.all(
    (await listAppDirs()).map(async ({ appId }) => {
      const hint = await loadAppHint(appId);
      return hint ? { appId, hint } : null;
    }),
  );
  // Already in id order: `listAppDirs` sorts, and `Promise.all` keeps positions.
  return hints.filter((h): h is { appId: string; hint: string } => h !== null);
}
