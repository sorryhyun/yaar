...
- `YAAR_REMOTE_TOKEN` - Adopt this remote token instead of minting one, so a launcher can build the `#remote=<token>` URL before the server starts (`scripts/dev.sh` does this for `make claude`). Under 32 chars it is ignored with a warning — the default tunnel is a public URL. See `http/auth.ts`.
- `YAAR_STORAGE` / `YAAR_CONFIG` - Override storage/config directory paths
- `YAAR_APP_ORIGIN_ISOLATION` - App-origin isolation (**on by default, local mode only**; set `=0` to disable). Pins the desktop to `localhost` and serves `source:'user'` app iframes from `127.0.0.1` so they are cross-origin to the desktop. **Enforcing:** `resolvePrincipal` refuses a token-less request that carries the app origin (a `127.0.0.1` `Origin`, or one landing on `127.0.0.1`), and `http/server.ts` redirects a desktop document that lands on `127.0.0.1` back to localhost. Closes the token-forgery escapes; being cross-origin also blocks `window.parent` DOM/memory reach, and top-level navigation is closed by the sandbox on isolated frames — see [`docs/guides/remote_mode.md`](../../docs/guides/remote_mode.md).
