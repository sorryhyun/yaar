...
**Claude failure channels (`providers/claude/errors.ts`):** the SDK reports trouble on five channels and only one of them is terminal. `assistant.error` (a typed `SDKAssistantMessageError`), `system/api_retry`, `system/permission_denied`, `system/model_refusal_*` and `rate_limit_event` all become **`StreamMessage.type === 'notice'`** — non-terminal by construction, because `StreamToEventMapper.map`'s `error` case calls `fail()`, which latches the turn closed, and a `rate_limit` is normally followed by a retry and a good answer. Only the SDK's `result` terminal stays an `error`, and its text is now assembled from `terminal_reason` and `stop_reason` when `errors[]` is empty (it usually is — that is where "Unknown SDK error" came from). Notices reach the client as `ServerEventType.AGENT_NOTICE` (a CLI-panel line, never `connectionError` or a failed message) and as a `notice` frame on `yaar://agents/{id}/stream`. The two code→sentence tables are total over the SDK's unions, so a CLI bump that adds a code breaks the build rather than degrading to the code name. Covered by `tests/claude-error-notices.test.ts`.
