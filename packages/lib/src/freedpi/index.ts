 ...
 *
 * Wired by the server's lifecycle, consumed by its `lib/browser/chrome.ts` (as
 * `--proxy-server`) and by `ssrf.ts` (as `fetch`'s `proxy` option). Being on costs an
 * unblocked network one loopback hop and nothing else: `policy.ts` starts every host
 * on the direct path and only an injected-looking reset moves it up a rung.
 */
