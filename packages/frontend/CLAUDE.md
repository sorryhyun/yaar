...
- Decomposed into `hooks/use-agent-connection/`: `connection` (socket lifecycle — `connect`/`disconnect`/`retryConnection`/`recoverAfterResume`, the liveness probe, the inbound handler), `commands` (every outbound frame, React-free), `transport-manager`, `server-event-dispatcher`, `outbound-command-helpers`, `usePendingEventDrainer`, `useMonitorSync`, `useClientPresence`
