...
export const createCliSlice: SliceCreator<CliSlice> = (set, _get) => ({
  ...
  toggleCliMode: () => set((state) => {
    state.cliMode = !state.cliMode
  }),

  addCliEntry: (entry) => set((state) => {
  ...
  updateCliStreaming: (agentId, content, type, monitorId) => set((state) => {
  ...
  finalizeCliStreaming: (agentId) => set((state) => {
  ...
  clearCliHistory: (monitorId) => set((state) => {
    if (monitorId) {
      state.cliHistory[monitorId] = []
    } else {
      state.cliHistory = {}
