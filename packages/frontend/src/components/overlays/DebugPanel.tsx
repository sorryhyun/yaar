...
import styles from '@/styles/overlays/DebugPanel.module.css';
            ...
            return (
              ...
              >
                <div className={styles.entryHeader}>
                  <span className={styles.direction}>{entry.direction === 'out' ? '→' : '←'}</span>
                  <span className={styles.type}>{entry.type}</span>
                  {summary && <span className={styles.summary}>{summary}</span>}
                  <span className={styles.time}>{formatTime(entry.timestamp)}</span>
                </div>
                {isExpanded && <pre className={styles.data}>{formatData(entry.data)}</pre>}
              </div>
            );
          })
        )}
