...
`make claude` 또는 `make codex`로 실행하거나, 번들된 실행 파일(`.exe`)을 사용하면 원격 모드가 자동으로 활성화됩니다. 서버가 시작될 때 일회용 인증 토큰이 생성되고, 터미널에 접속 URL과 QR 코드가 표시됩니다. QR 코드에는 `http://<LAN IP>:<PORT>/#remote=<token>` 형태의 URL이 인코딩되어 있으며, 핸드폰 카메라로 스캔하면 토큰 인증까지 자동으로 처리되어 바로 접속됩니다.

같은 네트워크 내의 기기(핸드폰, 태블릿, 다른 PC)는 별도 설정 없이 접속 가능합니다. 외부 인터넷에서도 공유기 포트포워딩을 설정하면 접속할 수 있지만, HTTPS를 지원하지 않으므로 Cloudflare Tunnel 같은 터널링 서비스를 사용하는 것을 권장합니다. 자세한 내용은 [원격 접속 가이드](./docs/remote_mode.md)를 참고하세요.
