...
/**
 * Resolve the real, displayable image URL for an <img>, accounting for
 * DCinside lazy-load attributes and placeholders. Returns '' when there is no
 * usable remote URL (placeholder/blank), or a `data:` URI if already inlined.
 */
export function extractRealImageUrl(img: HTMLImageElement): string {
  const cur = (img.getAttribute('src') ?? '').trim();
  if (cur.startsWith('data:') && !isPlaceholderSrc(cur)) return cur;
  let url = cur;
  if (isPlaceholderSrc(cur)) {
    const resolved = resolveLazyUrl(img);
    if (resolved && !isPlaceholderSrc(resolved)) url = resolved;
  }
  url = normalizeUrl(url);
  if (!url || isPlaceholderSrc(url)) return '';
  return url;
}
 ...
 * placeholder. The browser path converts most images to base64 data URIs, but
 * this function resolves lazy attributes as a fallback for:
 *   - the fast HTTP path (no browser, images never converted), and
 *   - any images the browser path missed (timeout, errors).
 *
