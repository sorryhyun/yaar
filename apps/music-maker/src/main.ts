...
function App() {
  ...
  return html`
    <div class="app-root">
      ...
      <!-- Transport bar -->
      <div class="transport">
        <button class=${() => `btn-play ${isPlaying() ? 'playing' : ''}`} onClick=${handlePlay}>
          ${() => (isPlaying() ? '■' : '▶')}
        </button>
        ...
        <button class="y-btn y-btn-sm" onClick=${handleGenerate}>✨ Generate Melody</button>
        <button class="y-btn y-btn-sm" onClick=${handleRandomizeDrums}>🎲 Random Drums</button>
      </div>
        ...
        <div class="sidebar">
          <div class="sidebar-section">
            ...
            >
              ${() =>
                SCALES.map(
                  (s) => html`
                    <option value=${s} selected=${() => scale() === s}>${SCALE_LABELS[s]}</option>
                  `,
                )}
            </select>
          </div>
          ...
          <div class="sidebar-section">
            ...
            <select
              ...
              onChange=${(e: Event) =>
                setChordProgression((e.target as HTMLSelectElement).value as ChordProgression)}
            >
              ${() =>
                PROGRESSIONS.map(
                  (p) => html`
                    <option value=${p} selected=${() => chordProgression() === p}>${p}</option>
                  `,
                )}
            </select>
          </div>
          ...
          <div class="sidebar-section">
            ...
            <div style="font-size:11px;color:var(--yaar-text-muted);line-height:1.6">
              <div>
                Scale: <span style="color:var(--yaar-text)">${() => SCALE_LABELS[scale()]}</span>
              </div>
              <div>Prog: <span style="color:var(--yaar-text)">${chordProgression}</span></div>
              <div>
                Notes: <span style="color:var(--yaar-accent)">${() => melodyPattern().length}</span>
              </div>
              <div>
                Step:
                <span style="color:var(--yaar-accent)"
                  >${() => (currentStep() >= 0 ? currentStep() + 1 : '-')}/16</span
                >
              </div>
            </div>
          </div>
              ...
              TRACK_KEYS.map(
                (t) => html`
                  <div style="display:flex;align-items:center;gap:6px;margin-bottom:4px">
                    <span
                      style=${`width:8px;height:8px;border-radius:50%;background:${TRACK_COLORS[t]};display:inline-block`}
                    ></span>
