...
- **command(name, params, appId?, timeoutMs?)** — execute an IDE action (createProject, writeFile, compile, deploy, preview, previewScreenshot, viewPreview, resizePreview, protocolLog, describeUri, listUri, cloneApp, describeBundledLibrary, clearConsole, gitHistory, gitDiff, gitRestore, gitCheckpoint, etc.). Pass `appId` to drive a controllable app instead. Pass `timeoutMs` for slow commands — see Workflow.
