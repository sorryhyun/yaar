...
The two failure directions are not symmetric: a phrase misread as an address
navigates to a host that does not exist, while an address misread as a phrase only
costs an agent turn.
...
`context.ts` so neither has to import the other. `sse.ts -> live/state.ts` is also fine:
`state.ts` imports nothing.
...
sitting still by definition, so without a re-capture the canvas keeps showing the last
frame of the tab we left.
...
other target attaches without complaint and then emits **nothing at all**: input is
forwarded, the remote page really scrolls, and the canvas sits on whatever the seed last
painted. Reconnecting the socket or navigating the tab does not revive it. The server's
screencast handlers (`packages/server/src/websocket/screencast-handlers.ts`) therefore
call `bringToFront()` before `startScreencast` on open and on tab switch, tolerating
failure.

`live/fallback.ts` is the client's safety net for stalls activation cannot prevent
(another tab taking the foreground mid-view, e.g. an agent driving it). The still
endpoint *does* answer for a background target, so when input has been forwarded and no
frame has answered it for ~900 ms, it forces a `fresh` capture of that tab. Usually the
capture itself does not reach the canvas: forcing it makes the target rasterize, Chrome
emits a real frame, and `seed.ts` drops the stale still. Either way the page visibly
moves, at a few frames per second.
...
the next frame, so a stream that goes quiet does not leave the window open and divide
its next frame by the whole idle span.

Two more rules: an input mark expires (`LAG_EXPIRY_MS`), so one
...
2. **Init script**: `web.setInitScript` installs `initScript` (`adblock-script.ts`) to run
   ...
   covers only `window.open` and `onbeforeunload`, the two hooks that lose a race if
   installed late: a popunder binds `window.open` during load, so a post-load override
   misses it.
...
and is shielded before its first script runs. The consequence: they are set from the
...
- **Everything must be reversible.** `APPLY` records each element's previous inline
  style on a `window.__yaarAdBlock` ledger and `DISABLE` walks it back. `INIT` shares
  that ledger: it stores the real `window.open` in `st.openOrig`, and `DISABLE` restores
  it from there. Turning the shield off restores the page in place, without a reload.
  Anything added to a payload has to join the ledger.
...
- **Popups are recorded, never auto-closed.** The server announces a popup on its
  *opener's* SSE stream (`popup` field, consumed in `sse.ts` ahead of the version gate
  because it does not advance `version`), with the opener Chrome named, which is more
  reliable than the in-page counter. It lands in `popupTabs` state and the badge. It is
...
- **Nothing traverses this app.** No base64, no `Range` assembly, no Blob in the iframe's
  ...
  `httpFetch` response. Do not reintroduce a client-side fetch for downloads.
- **The page's own download button works.** This app cannot press it, but with download
  behavior armed, pressing it is captured like any other download and announced on the
  SSE stream as a `download`
  ...
  **The directory is the source of truth, not the CDP event.** `behavior: 'allow'` means
  ...
  directory *is* a completed download, watched with `fs.watch`. Keying records off the
  guid finds no such file and times out on completed downloads.
...
- **A stored PDF is opened by framing its storage URL**, which `sharedStorage.url()`
  builds with this app's iframe token. Framing the *original* URL does not work: the
  desktop probes `/api/embeddable` first, and a host that refuses framing is routed
  straight back to this app. This step cannot be verified from a Dev Tools preview;
  `openUrl` is a silent no-op for the preview principal.
...
so changing it is a visible protocol change even though no command changed.
