...
import { editorState } from './protocol';
...
/**
 * Build the editor into the mount element and wire it up.
 *
 * This app owns its DOM outright, so it takes `defineApp`'s imperative `view`
 * escape hatch: the protocol is declared (and registered) at module scope, and
 * everything that needs a live editor — including the controller the protocol
 * handlers read through `ctl()` — happens here, when there is one.
 *
 * Returns the teardown for the window closing.
 */
function mountEditor(el: HTMLElement): () => void {
  const store = new EditorStore();
  const ui = createEditorUI(el, store);
  ...
  const releaseEditor = (): void => {
    cleanupKeyboard();
    editMode.releaseActiveObjectUrl();
    creatorMode.destroy();
  };

  window.addEventListener('beforeunload', releaseEditor);
  ...
  return () => {
    window.removeEventListener('beforeunload', releaseEditor);
    releaseEditor();
  };
}

export default defineApp({
  id: 'video-editor-lite',
