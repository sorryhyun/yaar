...
function Section(props: { title: string; items: DailyPaperItem[] }) {
  return html`
    <section style="margin-top: 12px;">
      <h3 style="margin: 0 0 10px; font-size: 15px; color: #cfd8e6;">
  ...
  () => html`
    <div class="wrap y-app">
      <div class="top">
        ...
        <select
          onChange=${(e: Event) => {
            setSourceMode((e.target as HTMLSelectElement).value as any);
            loadPapers();
          }}
        >
        ...
        <select
          onChange=${(e: Event) => {
            setLimitVal(Number((e.target as HTMLSelectElement).value));
            loadPapers();
          }}
        >
        ...
        <select
          onChange=${(e: Event) => {
            setDayRange((e.target as HTMLSelectElement).value);
            applyFiltersAndSort();
          }}
        >
        ...
        <button class="recommend-btn" onClick=${() => requestRecommendationsFromAgent('button')}>
          Recommend 2 papers
        </button>
        <button
          class="chart-btn"
        ...
        >
          📊 ${() => (chartOpen() ? 'Hide Chart' : 'Activity Chart')}
        </button>
      ...
      <div style="margin-top: 10px;">
        ${() => {
          ...
          const p = papers();
          if (p.length === 0)
            return html`<div class="y-empty empty">No papers available for this filter.</div>`;
          const mode = sourceMode();
          if (mode === 'both') {
