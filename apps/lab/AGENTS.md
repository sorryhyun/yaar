...
src/styles/            one sheet per area, imported in cascade order by styles/index.ts
```
...
- **`src/styles/` import order is the cascade.** `styles/index.ts` lists the sheets in
  the order they must apply; reordering the imports reorders the rules.
...
Every execution that arrives over the protocol must leave a trace in the window. Two
things keep it that way:

1. **The kernel sends UI parts for agent runs too** (`source/run-loop.ts`; do not
   reintroduce `msg.agent ? [] : __labS.parts`). `parts` cross the worker boundary, not
   the protocol boundary, and `__labEncode` has already capped them. The agent-facing
   `out.agent` summary is what `runCode` returns.
2. **`runCode` calls `logAgentRun`** (`src/protocol/run.ts`), and `state/run.ts` takes a
   `RunOrigin` so `runCell`/`runAll` know they were called by an agent.
...
- `mainView` is `'notebook' | 'agent'` — the log is a **sibling view**, full pane, switched
  by the tabs in the toolbar, not a panel stacked under the cells.
...
- Entries render through `OutputView`, the same component a cell uses. Do not fork it.
...
`scales.x.type` makes Chart.js fall back to a linear axis and render category labels as
0,1,2,3. `buildConfig` (`src/lib/chart-config.ts`) always names a concrete `type`.
Horizontal bars swap which axis carries the categories; that is the `o.horizontal` branch.
...
- A JS function input cannot cross, so the kernel samples it eagerly over `[xMin, xMax]` and
  ...
  difference. Do not try to make these equivalent.
...
Two GraphView invariants that are easy to break:
...
the notebook and returns `{ path, uri, bytes, cellId, kind }`. Keep it one command, so the
caller need not know which kind a cell produced.
...
`remove`/`exists` at once. Do not let an operation resolve on its own.
...
- `..` is refused in every form (server-side it resolves out of `apps/lab/` into
