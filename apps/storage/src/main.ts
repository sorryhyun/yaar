export {};
import { signal, html, css, mount, show } from '@bundled/yaar';
...
const entries = signal<StorageEntry[]>([]);
const mountAliases = signal<string[]>([]);
...
const previewTitleText = signal('Preview');
const previewMetaText = signal('');

// ── Refs ──────────────────────────────────────────────────────────────
let elMountAlias!: HTMLInputElement;
let elMountHostPath!: HTMLInputElement;
let elMountReadonly!: HTMLInputElement;
let elPreviewBody!: HTMLDivElement;
...
async function selectFile(entry: StorageEntry) {
  ...
  // elPreviewBody ref is always in DOM (class-toggled, not show/hide)
...
mount(html`
  ...
  <div class="main">
    ...
    <div class=${() => `preview${showPreview() ? '' : ' hidden'}`}>
      ...
      <div class="preview-body" ref=${(el: HTMLDivElement) => { elPreviewBody = el; }}></div>
      ...
      <div class="modal-card y-card">
        ...
        <form class="modal-form" onSubmit=${submitMountRequest}>
          ...
          <input class="modal-input y-input" name="alias" placeholder="project-files" required
            ref=${(el: HTMLInputElement) => { elMountAlias = el; }} />
          ...
          <input class="modal-input y-input" name="hostPath" placeholder="/Users/name/projects" required
            ref=${(el: HTMLInputElement) => { elMountHostPath = el; }} />
          <label class="modal-check">
            <input type="checkbox" name="readOnly"
              ref=${(el: HTMLInputElement) => { elMountReadonly = el; }} />
            Read-only mount
          </label>
