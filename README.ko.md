...
- **폴더 하나 = 앱 하나.** 스킬, 플러그인, 에이전트, UI가 하나의 폴더 규약으로 통일됩니다: 메타데이터(`app.json`), AI가 읽는 설명(`SKILL.md`), 전용 에이전트 정의(`AGENTS.md`, 선택), 그리고 단일 HTML 파일로 빌드되는 소스. 폴더를 넣으면 설치, 지우면 제거 — 등록 코드가 없습니다.
...
- **앱 오리진 격리** (로컬 모드 기본 활성화) — 설치된 앱은 별도의 브라우저 오리진(데스크톱은 `localhost`, 앱은 `127.0.0.1`)에서 제공되어, 앱이 토큰을 생략하고 데스크톱 행세를 할 수 없습니다. `YAAR_APP_ORIGIN_ISOLATION=0`으로 끌 수 있습니다.

**알려진 한계:** 오리진 경계는 로컬 모드에서만 존재합니다. **리모트 모드**(또는 `YAAR_APP_ORIGIN_ISOLATION=0`)에서는 앱이 데스크톱과 같은 오리진으로 제공되므로, *악성 설치 앱*이 `window.parent`로 데스크톱 DOM에 직접 접근할 수 있습니다. 리모트로 실행할 때는 신뢰할 수 없는 앱을 설치하지 마세요. [리모트 모드 → 앱 오리진 경계](./docs/ko/remote_mode.md#리모트-모드는-앱-오리진-경계를-포기합니다)를 참고하세요.
