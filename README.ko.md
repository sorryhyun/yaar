...
**알려진 한계:** 앱 iframe은 아직 샌드박스 없이 실행되므로, **악성 앱은 `window.parent`로 데스크톱 DOM에 직접 접근할 수 있습니다** — 오리진 격리는 토큰 위조 탈출은 막지만 이것은 막지 못합니다. 신뢰할 수 없는 앱은 설치하지 마세요. 이미 적용된 경계와 남은 샌드박스 작업은 [known gaps](./docs/architecture/known_gaps.md)를 참고하세요.
